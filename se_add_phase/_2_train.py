...
def main():
  ...
  # print(device)
  mp_model = model.MODEL(PARAM.MODEL_TRAIN_KEY, device)
  ...
  if len(ckpt_lst) > 0:
    ...
    for name, param in mp_model.named_parameters():
    ...
    evalOutputs_prev = eval_one_epoch(mp_model, val_batch_iter)
    misc_utils.print_log("                                            "
                         "                                            "
                         "                                         \n",
  ...
  for epoch in range(s_epoch, PARAM.max_epoch+1):
    ...
    # train
    trainOutputs = train_one_epoch(mp_model, train_batch_iter, train_log_file)
    ...
    # validation
    evalOutputs = eval_one_epoch(mp_model, val_batch_iter)
    ...
    mp_model.save_every_epoch(str(ckpt_dir.joinpath(ckpt_name)))
