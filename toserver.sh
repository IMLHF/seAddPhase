...
if [ "$site" == "p40" ]; then
  rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.gitignore' ./se_add_phase/* zhangwenbo5@ksai-P40-2:/home/zhangwenbo5/lihongfeng/seAddPhase/$2
elif [ "$site" == "v100-3" ]; then
  rsync -avh -e "ssh -p 22 -o ProxyCommand='ssh -p 8695 zhangwenbo5@120.92.114.84 -W %h:%p'" --exclude-from='.gitignore' ./se_add_phase/* zhangwenbo5@ksai-v100-3:/home/zhangwenbo5/lihongfeng/seAddPhase/$2
elif [ "$site" == "15123" ] || [ "$site" == "15041" ] || [ "$site" == "15043" ]; then
  echo "To $user@$site:~/worklhf/seAddPhase/$2"
  rsync -avh -e 'ssh -p '$site --exclude-from='.gitignore' ./se_add_phase/* $user@speaker.is99kdf.xyz:~/worklhf/seAddPhase/$2
elif [ "$site" == "15043ali" ]; then
  echo "To $user@$site:~/worklhf/seAddPhase/$2"
  rsync -avh -e 'ssh -p 6662' --exclude-from='.gitignore' ./se_add_phase/* $user@47.92.169.196:~/worklhf/seAddPhase/$2
